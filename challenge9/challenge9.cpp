...
void loop(string, vector<string>&, vector<double>&, vector<bool>&);
                    ...
                    //do all the calculations
                        if(input_string.find("=") != string::npos){
                            if(input_string.find("IF") != string::npos){
                                ...
                                loop(input_string, variable_names, variable_values, is_double);
                            }
                            else{
...
void loop(string input, vector<string>& name_vec, vector<double>& value_vec, vector<bool>& is_double){
    
    int iterator;

    stringstream convert;

    string statement = input.substr(input.find("LOOP") + 4, input.find("TIMES") - input.find("LOOP") - 4);
    string iterator_str = input.substr(input.find("TIMES") + 5);

    convert.clear();
    convert.str(iterator_str);
    convert>>iterator;

    for(int i = 0; i < iterator; i++){
        calculate(statement, name_vec, value_vec, is_double);
    }
