...
void calculate(string a, vector<string> &vec1, vector<int> &vec2){
    ...
    if(a.find('=') != string::npos){
        ...
        if(right_side.find('+') != string::npos){
            ...
            right_stream.clear(); right_stream.str(right_of_plus);
            left_stream.clear(); left_stream.str(left_of_plus);

            int left_of_plus_int, right_of_plus_int;
            right_stream>>right_of_plus_int;
            left_stream>>left_of_plus_int;

            if(index_left_of_plus != -1 && index_right_of_plus != -1){
                vec2[search(left_side, vec1)] = add(vec2[index_left_of_plus], vec2[index_right_of_plus]);
            }
            else if(index_left_of_plus == -1 && index_right_of_plus != -1){
                vec2[search(left_side, vec1)] = add(left_of_plus_int, vec2[index_right_of_plus]);
            }
            else if(index_left_of_plus!= -1 && index_right_of_plus == -1){
                vec2[search(left_side, vec1)] = add(vec2[index_left_of_plus], right_of_plus_int);
            }
            else{
                vec2[search(left_side, vec1)] = add(left_of_plus_int, right_of_plus_int);
            }
        }
    }

    if(a.find("OUT") != string::npos){
        string output_str = a.substr(a.find("OUT") + 3);
        cout<<vec2[search(output_str, vec1)]<<endl;
        OutputFile<<vec2[search(output_str, vec1)]<<endl;
    }
